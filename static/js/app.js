...
function login(){
    ...
    $.getJSON({
        url: "/login-cb", data: {'username': uName, 'pwd': pwd, 'buttonState': loginButton.value}, success: (res) => {
            if (res.isAuth){
                document.getElementById("username-in").value = '';
    ...
    $.getJSON({
        url: "/plot-update-cb", data: {'sess': sessValue, 'unit': unitValue, 'aMinX': aMinX, 'aMaxX': aMaxX, 'sMinX': sMinX, 'sMaxX': sMaxX}, success: (res) => {
            ...
            if (keys[0]==='refresh'){
                ...
                const arrNames = ["hh-array", "hl-array", "lh-array", "ll-array"];
                const saccNames = ["hh-sacc", "hl-sacc", "lh-sacc", "ll-sacc"];
                let arrUpdate = {'xaxis.range': [aMinX, aMaxX]};
                let saccUpdate = {'xaxis.range': [sMinX, sMaxX]};
                var i;
                for (i=0; i < arrNames.length; i++){
                    Plotly.relayout(document.getElementById(arrNames[i]), arrUpdate);
                    Plotly.relayout(document.getElementById(saccNames[i]), saccUpdate);
                }
            } else {
