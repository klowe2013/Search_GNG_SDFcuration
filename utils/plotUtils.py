...
def PlotConds(mu_dict, sem_dict, cond, show_in=True, show_out=True, fig=None):
    ...
    if show_in:
        PltMeanStd(mu_dict['t'], mu_dict[cond]['in'], sem_dict[cond]['in'],
                   fig=fig, color=colors[cond], width=IN_WIDTH, name=cond.upper()+' IN')
    if show_out:
        PltMeanStd(mu_dict['t'], mu_dict[cond]['out'], sem_dict[cond]['out'],
...
def PlotPop(pop_sdfs, cond, show_in=True, show_out=True, fig=None):
    ...
    if show_in:
        PltMeanStd(pop_sdfs['Times'],
                   ...
                   np.nanstd(pop_sdfs[cond.upper()]['in'], axis=0) /
                   np.sqrt(pop_sdfs[cond.upper()]['in'].shape[0]),
                   fig=fig, color=colors[cond], width=IN_WIDTH, name=cond.upper()+' IN')
    if show_out:
        PltMeanStd(pop_sdfs['Times'],
                   np.nanmean(pop_sdfs[cond.upper()]['out'], axis=0),
                   np.nanstd(pop_sdfs[cond.upper()]['out'], axis=0) /
                   np.sqrt(pop_sdfs[cond.upper()]['out'].shape[0]),
                   fig=fig, color=colors[cond], width=OUT_WIDTH, name=cond.upper()+' OUT')
...
def AddVLine(fig, cond, sst_dict, mov=False):
    ...
    sel_colors = {'sst': 'rgb(54,201,54)', 'cdt': 'rgb(54, 201, 201)',
                  'ngsst': 'rgb(230, 147, 23)', 'outcdt': 'rgb(201, 201, 54)', 'unk': 'rgb(0,0,0)'}
    # Get Y lim
